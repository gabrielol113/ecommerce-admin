...
export default function Nav({show}){
    ...
    const activeLink = inactiveLink+' bg-highlight text-black rounded-l-lg mr-0';
    async function logout() {
        await router.push('/');
        await signOut();
    }
    return(
        <aside className={`${(show? 'left-0 top-20':'-left-full')} top-10 text-gray-500 p-4 fixed w-full bg-bgGray h-full md:static md:w-auto transition-all pr-0 mr-2`}>
